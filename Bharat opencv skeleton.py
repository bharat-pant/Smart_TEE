...
def lane_check_1(q1,fg1):
    ...
    ######################################################################### last commit made a list to store max value
        ...
        if cv2.waitKey(33) == 27 or max(set_lane_1)>10: #threshold ki jagah 10 dala hai
            break
    fg1.put(1)
...
def lane_check_2(q2,fg2):
    ...
    ###############################################################commit change 2
        for (x,y,w,h) in cars:
            cv2.rectangle(frames,(x,y),(x+w,y+h),(0,0,255),2)
        cv2.imshow('video2', frames)                                # Display frames in a window
        if cv2.waitKey(33) == 27 or max(set_lane_2)>10:   #threshold ki jagah 10 dala hai                                   # Wait for Esc key to stop
            break
    fg2.put(1)
...
def lane_check_3(q3,fg3):
    ...
    while True:
        ...
        cv2.imshow('video2', frames)                                # Display frames in a window
        if cv2.waitKey(33) == 27 or max(set_lane_3)>10:            #threshold ki jagah 10 dala hai                       # Wait for Esc key to stop
            break
    fg3.put(1)
    cv2.destroyAllWindows()                                         # De-allocate any associated memory usage
...
def lane_check_4(q4,fg4):
    ...
    while True:
        ...
        cv2.imshow('video2', frames)                                # Display frames in a window
        if cv2.waitKey(33) == 27 or max(set_lane_4)>10:            #threshold ki jagah 10 dala hai                       # Wait for Esc key to stop
            break
    fg4.put(1)
    cv2.destroyAllWindows()                                         # De-allocate any associated memory usage
